...
data_train = load("measurement_data\prbs_train_121s_amp_015.mat");
data_val = load("measurement_data\prbs_validation_121s_amp_015.mat");
...
fprintf('\n======================================================\n');
fprintf('PASS 1: Estimating active linkage parameters...\n');
fprintf('======================================================\n');

% Pass 1:
% Parameters 1:6 are free.
% Parameters 7:13 are locked according to fixed_flags.
model_est_pass1 = nlgreyest(iddata_train, model_train_initial, opt);

fprintf('\nPass 1 completed.\n');
fprintf('Pass 1 Loss Function: %g\n', model_est_pass1.Report.Fit.LossFcn);

fprintf('\n======================================================\n');
fprintf('PASS 2: Re-estimating originally locked parameters...\n');
fprintf('======================================================\n');

% Use the pass-1 result as the starting point for pass 2.
model_pass2_initial = model_est_pass1;

% Pass 2:
% - Parameters that were free in pass 1 are now fixed.
% - Parameters that were locked in pass 1 are now released.
for k = 1:length(parameter_names)
    if fixed_flags(k)
        model_pass2_initial.Parameters(k).Fixed = false;
    else
        model_pass2_initial.Parameters(k).Fixed = true;
    end
end

% Keep initial conditions fixed during pass 2 as well.
model_pass2_initial = setinit(model_pass2_initial, 'Value', initial_states_train);
model_pass2_initial = setinit(model_pass2_initial, 'Fixed', {true; true; true; true});

% Use a separate options object for the second pass, so this can be tuned later
% without changing the first active-parameter estimation pass.
opt_pass2 = opt;
opt_pass2.SearchOptions.MaxIterations = 100;

model_est_pass2 = nlgreyest(iddata_train, model_pass2_initial, opt_pass2);

fprintf('\nPass 2 completed.\n');
fprintf('Pass 2 Loss Function: %g\n', model_est_pass2.Report.Fit.LossFcn);

% The final model is the result of the second pass.
model_est = model_est_pass2;
...
try
    ...
    std_devs_final = nan(num_params, 1);

    % getcov returns covariance for the parameters that were free in the
    % final estimation pass. Parameters fixed in the final pass therefore
    % do not receive a final-pass standard deviation here.
    cov_idx = 1;

    for k = 1:num_params
        if ~model_est.Parameters(k).Fixed
            std_devs_final(k) = sqrt(cov_matrix(cov_idx, cov_idx));
            cov_idx = cov_idx + 1;
        end
    end

    for k = 1:num_params
        if isnan(std_devs_final(k))
            fprintf('%-10s : %12.6f    fixed in final pass\n', ...
                model_est.Parameters(k).Name, ...
                model_est.Parameters(k).Value);
        else
            fprintf('%-10s : %12.6f  +/- %-12.6f\n', ...
                model_est.Parameters(k).Name, ...
                model_est.Parameters(k).Value, ...
                std_devs_final(k));
        end
    ...
    for k = 1:num_params
        if model_est.Parameters(k).Fixed
            status_txt = 'fixed in final pass';
        else
            status_txt = 'estimated in final pass';
        end
